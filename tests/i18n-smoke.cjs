...
(async()=> {
 ...
 try {
   ...
   await page.locator('[data-language-option="NL"]').first().click();
   assert.equal(await page.locator('html').getAttribute('lang'),'nl');
   assert.equal(await page.locator('[data-language-current]').first().textContent(),'NL');
   assert.ok((await page.locator('body').innerText()).includes('Over mij'));
   ...
   for (const file of files) {
     ...
     const result = await page.evaluate(()=> {
       ...
       while(n=walk.nextNode()) {
         ...
         if (window.LUXIA_NL[value] && window.LUXIA_NL[value] !== value) untranslated.push(value);
       }
     ...
     await page.evaluate(()=>LuxiaI18n.setLanguage('nl'));
   ...
   assert.ok((await select.textContent()).includes('België'));
   assert.equal(await select.inputValue(),'+32');
   const names = await select.locator('option').allTextContents();
   assert.deepEqual(names,[...names].sort((a,b)=>a.replace(/^\S+\s*/,'').localeCompare(b.replace(/^\S+\s*/,''),'nl-BE')));
   await page.evaluate(()=>LuxiaI18n.setLanguage('en'));
   ...
   await page.waitForFunction(()=>document.querySelector('[data-auth-status]').textContent.includes('De wachtwoorden komen niet overeen.'));
   await page.goto(origin+base+'pages/book-consultation.html');
   await page.locator('[data-check-availability]').click();
   await page.waitForFunction(()=>/beschikbare tijdstippen/.test(document.querySelector('[data-calendar-status]').textContent));
   ...
   await page.waitForFunction(()=>document.querySelector('#dynamic-test').textContent==='De wachtwoorden komen niet overeen.');
   await page.evaluate(()=>sessionStorage.setItem('testUser','1'));
   await page.goto(origin+base+'pages/client-space.html');
   await page.waitForFunction(()=>document.body.textContent.includes('Betaling: Goedgekeurd'));
   assert.ok((await page.locator('body').innerText()).includes('De privésessie is verlopen.'));
   assert.ok((await page.locator('body').innerText()).includes('Geslacht: vrouw'));
   await page.locator('.luxia-loader').waitFor({state:'detached'});
   await page.screenshot({path:path.join(output,'nl-client-desktop.png'),fullPage:true});
   ...
   for(const file of ['index.html','pages/aboutme.html','pages/book-consultation.html','pages/create-account.html']) {
     await page.goto(origin+base+file);
     await page.locator('.luxia-loader').waitFor({state:'detached'});
     await page.screenshot({path:path.join(output,'nl-'+path.basename(file,'.html')+'-mobile.png'),fullPage:true});
     await page.screenshot({path:path.join(output,'nl-'+path.basename(file,'.html')+'-mobile-top.png')});
     assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+1),file+' mobile overflow');
   }
   assert.deepEqual(errors,[],'Browser errors');
   console.log(JSON.stringify({passed:true,pages:report,checks:['language button','persistence','roundtrip','form values','country names and sorting','dynamic status','booking status','mobile overflow']},null,2));
